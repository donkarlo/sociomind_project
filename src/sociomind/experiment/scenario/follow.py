...
class Follow(Scenario):
    def __init__(self):
        ...
        self._plan = FollowPlan(ActionSet([GoTo(Pose(Vec([1, 2, 3]), Vec([1, 2, 3, 4])))]))  # pose points from normal scenario points
        self._mission = SyncedTurningAroundCorridor()

        self._robots = Group.get_robots()

        super().__init__(self._robots, self._mission, self._world,
